...
namespace AchtungMod
{
    public class JobDriver_SowAll : JobDriver_Thoroughly
    {
        ...
        public override IEnumerable<LocalTargetInfo> CanStart(Pawn thePawn, LocalTargetInfo clickCell)
        {
            ...
            if (thePawn.workSettings == null || thePawn.workSettings.GetPriority(WorkTypeDefOf.Growing) == 0)
                return null;
            var cells = AllWorkAt(clickCell, thePawn, true);
            IEnumerable<LocalTargetInfo> result =
                cells != null && cells.Count() > 0 ? new List<LocalTargetInfo> {clickCell} : null;
        ...
        private bool CanSowHere(IntVec3 cell, ThingDef def, bool fastMode)
        {
            ...
            if (fastMode == false && pawn.CanReach(cell, PathEndMode.Touch, pawn.NormalMaxDanger()) == false)
                return false;
        ...
        public bool Harvest(Plant plant)
        {
            ...
            if (subCounter >= plant.def.plant.harvestWork)
            {
                ...
                {
                    if (pawn.RaceProps.Humanlike && plant.def.plant.harvestFailable &&
                        Rand.Value > pawn.GetStatValue(StatDefOf.PlantHarvestYield, true))
                    {
                    ...
                    else
                    {
                        ...
                        if (count > 0)
                        {
                            ...
                            t.stackCount = count;
                            if (pawn.Faction != Faction.OfPlayer) t.SetForbidden(true, true);
        ...
        public IEnumerable<Plant> PlantsToCutFirstAt(IntVec3 cell)
        {
            return cell.GetThingList(pawn.Map)
                .Where(p => p is Plant && p.def != currentPlantDef && p.def.BlockPlanting && p.Destroyed == false)
                .Cast<Plant>();
        }
