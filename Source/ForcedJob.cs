...
namespace AchtungMod
{
    ...
    public class ForcedJob : IExposable
    {
        ...
        public IEnumerable<LocalTargetInfo> GetSortedTargets(HashSet<int> planned)
        {
            ...
            return targets
                ...
                .OrderByDescending(target =>
                {
                    ...
                    var neighbourScore =
                        willBlock ? Tools.NeighbourScore(target.item.Cell, pathGrid, mapWidth, planned) : 100;
        ...
        public bool GetNextJob(out Job job, out LocalTargetInfo atItem)
        {
            ...
            if (exist)
                Find.LetterStack.ReceiveLetter("No forced work",
        ...
        public static bool ContinueJob(Pawn_JobTracker tracker, Job lastJob, Pawn pawn, JobCondition condition)
        {
            ...
            {
                Messages.Message("Forced work of " + pawn.Name.ToStringShort + " was interrupted.",
        ...
        {
            if (Scribe.mode == LoadSaveMode.Saving)
                _ = targets.RemoveWhere(target =>
                    target.item == null || target.item.IsValid == false || target.item.ThingDestroyed);
    ...
    public static class ForcedExtensions
    {
            ...
            bool ignoreReserve = false)
        {
            ...
            if (workgiver.PotentialWorkThingRequest.Accepts(thing) ||
                workgiver.PotentialWorkThingsGlobal(pawn) != null &&
                workgiver.PotentialWorkThingsGlobal(pawn).Contains(thing))
                if (workgiver.MissingRequiredCapacity(pawn) == null)
                    if (workgiver.HasJobOnThing(pawn, thing, true))
                    {
                        ...
                        if (job != null)
                            if (Achtung.Settings.ignoreForbidden && ignoreRestrictions ||
                                thing.IsForbidden(pawn) == false)
                                if (Achtung.Settings.ignoreRestrictions && ignoreRestrictions ||
                                    thing.Position.InAllowedArea(pawn))
                                {
                                    var ok1 = ignoreReserve == false &&
                                              pawn.CanReserveAndReach(thing, workgiver.PathEndMode, Danger.Deadly);
                                    var ok2 = ignoreReserve &&
                                              pawn.CanReach(thing, workgiver.PathEndMode, Danger.Deadly);
                                    if (ok1 || ok2)
                                        return job;
                                }
                    }
