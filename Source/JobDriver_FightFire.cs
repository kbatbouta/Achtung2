    ...
    public class JobDriver_FightFire : JobDriver_Thoroughly
    {
        ...
        public override IEnumerable<LocalTargetInfo> CanStart(Pawn thePawn, LocalTargetInfo clickCell)
        {
            ...
            if (thePawn.workSettings == null ||
                thePawn.workSettings.GetPriority(WorkTypeDefOf.Firefighter) == 0) return null;
            var item = thePawn.Map.thingGrid.ThingAt(clickCell.Cell, ThingDefOf.Fire);
            if (item == null) return null;
            var canFight = item.Destroyed == false &&
                           thePawn.CanReach(item, PathEndMode.Touch, thePawn.NormalMaxDanger()) &&
                           thePawn.CanReserve(item, 1);
        ...
        public override LocalTargetInfo FindNextWorkItem()
        {
            return workLocations
                ...
                .FirstOrDefault(f =>
                    f != null && f.Destroyed == false && pawn.CanReach(f, PathEndMode.Touch, pawn.NormalMaxDanger()) &&
                    pawn.CanReserve(f, 1));
        }
