...
namespace AchtungMod
{
    public class ForcedMultiFloatMenuOption : ForcedFloatMenuOption
    {
        ...
        public ForcedMultiFloatMenuOption(string label) : base(label, null, MenuOptionPriority.Default, null, null, 0f,
            null, null)
        {
        }
            ...
            foreach (var option in options)
                ...
                else
                {
                    if (option.ForceAction())
                    {
                        ...
                        if (jobItem != null)
                        {
                            sharedCells = jobItem.GetSortedTargets(new HashSet<int>()).Select(item => item.Cell)
                                .ToList();
                            result = true;
                        }
                    }
    ...
    public class ForcedFloatMenuOption : FloatMenuOption
    {
        ...
        public static TaggedString ForcedLabelText = Translator.CanTranslate("ForceMenuButtonText")
            ? "ForceMenuButtonText".Translate()
            : new TaggedString(" ! ");
        ...
        public static FloatMenuOption CreateForcedMenuItem(string label, Action action, MenuOptionPriority priority,
            Action mouseoverGuiAction, Thing revalidateClickTarget, float extraPartWidth,
            Func<Rect, bool> extraPartOnGUI, WorldObject revalidateWorldClickTarget, Pawn pawn, IntVec3 clickCell,
            WorkGiver_Scanner workgiver)
        {
            if (action == null)
                return new FloatMenuOption(label, action, priority, mouseoverGuiAction, revalidateClickTarget,
                    extraPartWidth, extraPartOnGUI, revalidateWorldClickTarget);

            var option = new ForcedFloatMenuOption(label, action, priority, mouseoverGuiAction, revalidateClickTarget,
                extraPartWidth, extraPartOnGUI, revalidateWorldClickTarget) { };
        ...
        public ForcedFloatMenuOption(string label, Action action, MenuOptionPriority priority,
            Action mouseoverGuiAction, Thing revalidateClickTarget, float extraPartWidth,
            Func<Rect, bool> extraPartOnGUI, WorldObject revalidateWorldClickTarget)
            : base(label, action, priority, mouseoverGuiAction, revalidateClickTarget, extraPartWidth, extraPartOnGUI,
                revalidateWorldClickTarget)
        {
            // somehow necessary or else 'extraPartWidth' will be 0
            this.extraPartWidth = buttonSpace + ButtonWidth;
        }
        ...
        public bool RenderExtraPartOnGui(Rect drawRect)
        {
            ...
            rect = new Rect(Mathf.Round(rect.x), Mathf.Round(rect.y), Mathf.Round(rect.width),
        ...
        public static bool ForceActionSynced(Pawn forcePawn, WorkGiver_Scanner forceWorkgiver, int x, int z)
        {
            ...
            foreach (var workgiverDef in workgiverDefs)
            {
                ...
                if (forcedWork.AddForcedJob(forcePawn, workgiverDefs, item))
                {
                    ...
                    var success = job == null
                        ? false
                        : forcePawn.jobs.TryTakeOrderedJobPrioritizedWork(job, workgiver, forceCell);
                    if (success == false)
                    {
                        forcedWork.Prepare(forcePawn);
                        continue;
            ...
            Messages.Message(
                forcePawn.Name.ToStringShort +
                " could not find more forced work. The remaining work is most likely reserved or not accessible.",
