...
namespace AchtungMod
{
    ...
    public static class AchtungLoader
    {
        ...
        static AchtungLoader()
        {
            ...
            IsSameSpotInstalled = harmony.GetPatchedMethods()
                .Any(method =>
                    Harmony.GetPatchInfo(method).Transpilers.Any(transpiler => transpiler.owner == sameSpotId));
        }
    }
    ...
    {
        public static bool CanReserve(ReservationManager reservationManager, Pawn claimant, LocalTargetInfo target,
            int maxPawns, int stackCount, ReservationLayerDef layer, bool ignoreOtherReservations)
        {
            ...
            return reservationManager.CanReserve(claimant, target, maxPawns, stackCount, layer,
        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            if (idx > 0)
            {
                if (list[idx - 2].opcode == OpCodes.Ldc_I4_1)
                    ...
                    Log.Error(
                        "Cannot find Ldc_I4_1 before Pawn.Notify_Teleported in Pawn_PathFollower.TryRecoverFromUnwalkablePosition");
            }
            else
            {
    ...
    {
        public static IEnumerable<Thing> RadialDistinctThingsAround_Patch(IntVec3 center, Map map, float radius,
            bool useCenter, Pawn pawn)
        {
            var forcedWork = Find.World.GetComponent<ForcedWork>();
        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            var m_RadialDistinctThingsAround =
                AccessTools.Method(typeof(GenRadial), nameof(GenRadial.RadialDistinctThingsAround));
            var m_RadialDistinctThingsAround_Patch = AccessTools.Method(
                typeof(WorkGiver_ConstructDeliverResources_FindNearbyNeeders_Patch),
                "RadialDistinctThingsAround_Patch");
            if (m_RadialDistinctThingsAround_Patch == null)
                throw new Exception(
                    "Cannot find method WorkGiver_ConstructDeliverResources_FindNearbyNeeders_Patch.m_RadialDistinctThingsAround_Patch");
            ...
            if (found != 2)
                Log.Error(
                    "Cannot find both calls to RadialDistinctThingsAround in WorkGiver_ConstructDeliverResources.FindNearbyNeeders");
    ...
    internal static class FloatMenuMakerMap_AddJobGiverWorkOrders_Patch
    {
        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            var c_FloatMenuOption =
                AccessTools.FirstConstructor(typeof(FloatMenuOption), c => c.GetParameters().Count() > 1);
            var m_ForcedFloatMenuOption = AccessTools.Method(typeof(ForcedFloatMenuOption),
            ...
            if (foundCount != 2)
                Log.Error(
                    "Cannot find 2x Pawn_WorkSettings.GetPriority in RimWorld.FloatMenuMakerMap::AddJobGiverWorkOrders");
                ...
                .DoIf(i => list[i].opcode == OpCodes.Isinst && (Type)list[i].operand == typeof(WorkGiver_Scanner), i =>
               {
                   ...
                   if (list[index].opcode == OpCodes.Stloc_S)
                   {
                       ...
                       index = list.FindIndex(index,
                           code => code.opcode == OpCodes.Newobj &&
                                   (ConstructorInfo)code.operand == c_FloatMenuOption);
                       if (index < 0)
                       {
                           Log.Error(
                               "Cannot find 'Isinst WorkGiver_Scanner' in RimWorld.FloatMenuMakerMap::AddJobGiverWorkOrders");
                       }
                       else
                       {
            ...
            if (foundCount != 2)
                Log.Error(
                    "Cannot find 2x 'Isinst WorkGiver_Scanner', 'Stloc_S n' -> 'Newobj FloatMenuOption()' in RimWorld.FloatMenuMakerMap::AddJobGiverWorkOrders");
        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            for (var i = 0; i < instrList.Count; i++)
            {
                ...
                if (instrList[i + 1].opcode != OpCodes.Ldarg_2 || instrList[i + 2].opcode != OpCodes.Brfalse &&
                    instrList[i + 2].opcode != OpCodes.Brfalse_S)
                {
    ...
    internal static class PriorityWork_GetGizmos_Patch
    {
        ...
        public static readonly Texture2D ForceRadiusShrinkOff =
            ContentFinder<Texture2D>.Get("ForceRadiusShrinkOff", true);
    ...
    // custom context menu
    //    
    [HarmonyPatch(typeof(FloatMenuMakerMap), nameof(FloatMenuMakerMap.ChoicesAtFor), new[] { typeof(Vector3), typeof(Pawn) })]
