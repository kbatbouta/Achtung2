...
namespace AchtungMod
{
    ...
    static class MentalBreaker_MentalExtreemBreakerTick_Patch
    {
        public static bool Prefix(Map __instance)
        {
            if (!Gen.IsHashIntervalTick(Find.FactionManager.OfPlayer, 243))
                return true;
    ...
    [HarmonyPatch(typeof(Map))]
    [HarmonyPatch(nameof(Map.MapPreTick))]
    static class MentalBreaker_MentalMajorBreakerTick_Patch
    {
        public static bool Prefix(Map __instance)
        {
            if (!Gen.IsHashIntervalTick(Find.FactionManager.OfPlayer, 91))
                return true;

            foreach (Pawn p in BreakRiskAlertUtility.PawnsAtRiskMajor)
            {
                if (p.Map != __instance)
                    continue;
    ...
    [HarmonyPatch(typeof(Map))]
    [HarmonyPatch(nameof(Map.MapPreTick))]
    static class MentalBreaker_HealtTick_Patch
    {
        public static bool Prefix(Map __instance)
        {
            if (!Gen.IsHashIntervalTick(Find.FactionManager.OfPlayer, 113))
                return true;

            foreach (Pawn p in __instance.mapPawns.FreeColonists)
            {

                //BreakRiskAlertUtility.

                if (!p.Starving() && !HealthAIUtility.ShouldBeTendedNowByPlayerUrgent(p))
                    continue;
